type Career = {
  period: string
  kind: string
  content: string
  url: string
}

// TODO: github.com/ShotaKitazawa/ShotaKitazawa で管理する
export const data: Career[] = [
  {
    period: '2021/05 ~ 2021/08',
    kind: 'イベント運営',
    content: 'ISUCON11 予選作問担当',
    url: 'https://isucon.net/archives/55821036.html',
  },
  {
    period: '2021/02 ~',
    kind: 'イベント運営',
    content: 'CloudNative Days: Dreamkast 開発',
    url: 'https://cloudnativedays.jp/',
  },
  {
    period: '2020/06 ~ 2020/09',
    kind: 'イベント運営',
    content: 'NTT Performance Tuning Challenge 2020 運営',
    url: 'https://www.ntt.com/index.html',
  },
  {
    period: '2019/04 ~ ',
    kind: '勤務',
    content: 'NTTコミュニケーションズ株式会社',
    url: 'http://archive.interop.jp/2018/',
  },
  {
    period: '2018/06',
    kind: 'イベント運営',
    content: 'Interop Tokyo 2018 STM',
    url: 'http://archive.interop.jp/2018/',
  },
  {
    period: '2019/03 ~ 2019/04',
    kind: 'Internship',
    content: '株式会社サイバーエージェント',
    url: '',
  },
  {
    period: '2018/10 ~ 2018/12',
    kind: 'Internship',
    content: 'グリー株式会社',
    url: '',
  },
  {
    period: '2015/04 ~ 2019/03',
    kind: '学生団体',
    content: 'CIS RAT',
    url: 'https://rat.cis.k.hosei.ac.jp/article/rat/aboutrat.html',
  },
]
